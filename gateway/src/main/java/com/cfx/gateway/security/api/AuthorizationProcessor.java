...
public interface AuthorizationProcessor extends Ordered {
    ...
    /**
     * 前置处理器，在 {@link AuthorizationProcessor#process(Mono)} 执行前执行
     *
     ...
     */
    @Nullable
    Mono<Authorization> process(@NonNull Mono<Authorization> authorizationMono);

    /**
     * 后置处理器 在 {@link AuthorizationProcessor#process(Mono)} 后执行
     *
