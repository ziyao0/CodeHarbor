...
import com.ziyao.harbor.crypto.symmetric.SymmetricAlgorithm;
...
public abstract class KeyUtils {
    ...
    public static String getMainAlgorithm(String algorithm) {
        ...
        final int slashIndex = algorithm.indexOf(Chars.SLASH);
        if (slashIndex > 0) {
    ...
    public static SecretKey generateKey(String algorithm, byte[] key) {
        ...
        if (algorithm.startsWith("PBE")) {
            // PBE密钥
            secretKey = generatePBEKey(algorithm, (null == key) ? null : Strings.toString(key, Charsets.CHARSET_UTF_8).toCharArray());
